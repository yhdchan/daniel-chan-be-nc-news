  ...
  if (err.code === "22P02") {
    ...
    if (idCheck === "$1") {
      ...
      if (typeof req.body.inc_votes === "number") {
        res.status(400).send({
          msg: "Bad request! Increment value is not an integer! The increment value must be a non-zero integer.",
        });
      } else {
        res.status(400).send({
          msg: "Bad request! Wrong data type! The increment value must be a non-zero integer.",
        });
      }
    } else {
